...
export default async function handler(req, res) {
  ...
  const sourcesParam = req.query?.sources || "naver,youtube";
  const enabled = sourcesParam.split(",").map((s) => s.trim());

  try {
    const [nResult, yResult] = await Promise.allSettled([
    ...
    if (!nPosts.length && !yPosts.length) {
      return res.status(404).json({ error: `"${keyword}" 관련 글을 찾지 못했습니다.` });
    }

    const { ppData, collectedPosts } = buildCombinedPPData(keyword, nPosts, yPosts);
